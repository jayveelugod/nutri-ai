...
async def analyze_food(
    ...
    if image:
        ...
        # Build user-specific upload directory: uploads/{userId}/{userName}
        user_dir = os.path.join(UPLOAD_DIR, str(current_user.id), current_user.name)
        ...
        # URL path that will be stored in db
        image_url = f"/uploads/{current_user.id}/{current_user.name}/{filename}"
